...
ForEach ($GroupArrayItem in $GroupArray)
 {
    ...
    IF ( ($GroupArrayItem -eq 'IIS_IUSRS') -AND ($ADGroupMemberArray.Count -ge 1) )
     { Write-Warning "The $GroupArrayItem group should typically empty, but contains $($ADGroupMemberArray.Count) members" }
 }
